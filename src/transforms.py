...
class AudioPreprocessor(nn.Module):
    ...
    def forward(self, x, labels=None, augment=False, noise_prob=0.8, is_train=True):
        ...
        if augment and self.background_noise:
            ...
            for idx in range(x.shape[0]):
                ...
                if current_label != 0 and (not is_train or random.random() > noise_prob):
                    pass # Skip noise injection
                else:
                    # Amplitude: Low (0.1) for speech, High (1.0) for silence to vary texture
                    noise_amp = np.random.uniform(0, 0.1) if current_label != 0 else np.random.uniform(0, 1)
                    
                    # Select random noise
                    noise_waveform = random.choice(self.background_noise).to(self.device)
                    
                    # Random crop or tile to match input length
                    if noise_waveform.shape[-1] >= x.shape[-1]:
                        sample_loc = random.randint(0, noise_waveform.shape[-1] - x.shape[-1])
                        noise_segment = noise_waveform[:, sample_loc : sample_loc + x.shape[-1]]
                    else:
                        repeats = 1 + x.shape[-1] // noise_waveform.shape[-1]
                        noise_segment = noise_waveform.repeat(1, repeats)[:, :x.shape[-1]]
    ...
    @staticmethod
    def apply_spec_augment(x, freq_mask_para, time_mask_para, freq_mask_num, time_mask_num):
