...
VOCAB_CONFIGS = {
    ...
    24: [
        ...
        "backward", "forward", "follow", "learn", "zero", "one", "two", "three",
...
def main():
    ...
    print(
        f"Data Loaded - Train: {len(df_train)} | Valid: {len(df_valid)} | Test: {len(df_test)} | Noise: {len(df_noise)}"
    )
    ...
    for N in N_SET:
        ...
        for TAU in TAU_SET:
            ...
            for HOP_LENGTH in HOP_LENGTH_SET:
                ...
                # Wrapped Loaders
                train_iter = PreprocessingDataLoader(
                    train_loader, preprocess_train, is_train=True
                )
                valid_iter = PreprocessingDataLoader(
                    valid_loader, preprocess_eval, is_train=False
                )
                ...
                model = BCResNets(
                    int(TAU * 8), num_classes=(len(target_classes) + 2)
                ).to(DEVICE)
                ...
                optimizer = torch.optim.SGD(
                    model.parameters(), lr=0, weight_decay=1e-3, momentum=0.9
                )
                scheduler = BCResNetScheduler(
                    optimizer,
                    len(train_loader),
                    TOTAL_EPOCH,
                    WARMUP_EPOCH,
                ...
                early_stopping_path = os.path.join(
                    MODEL_DIR, f"temp_checkpoint_{config_name}.pt"
                )
                early_stopping = EarlyStopping(
                    patience=PATIENCE, verbose=True, path=early_stopping_path
                )
                ...
                print(
                    f"Training finished in {int(duration // 3600):02d}:{int((duration % 3600) // 60):02d}:{int(duration % 60):02d}"
                )

                # Save Final Model and History
                torch.save(
                    model.state_dict(),
                    os.path.join(MODEL_DIR, f"bcresnet_{config_name}.pt"),
                )
                torch.save(
                    {
